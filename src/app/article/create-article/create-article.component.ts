...
export class CreateArticleComponent implements OnInit {
  ...
  get editorConfig() {
    return {
      ...
      image: { toolbar: ['imageTextAlternative', 'uploadImage']}
    };
  }
  ...
  submitArticle() {
    if (this.Article.tittle.length < 30) {
      ...
      this.toastr.warning(
        'Desciption must be longer than 220 character',
        'Description'
      );
    } else {
      this.isLoaded = false;
      ...
      this.createArticleService.submitArticle(this.Article).subscribe(
        (res) => {
          console.log(res);
        ...
        (err) => {
          this.toastr.error(
            'Something went wrong. Make sure all field is filled',
            'Article'
          );
        }
      );
